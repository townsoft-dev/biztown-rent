...
| BR-BILL-08 | Vào/ra giữa tháng: **tiền nhà** chia theo số ngày ở thực tế, áp dụng ở kỳ đầu (từ ngày ghi nhận `MOVE_IN`) và kỳ cuối (đến ngày ghi nhận `MOVE_OUT`). **Điện/nước không cần chia** — mốc chỉ số nhận/trả phòng đã phản ánh đúng lượng dùng thực tế của từng bên | |
...
| BR-READ-05 | Không áp dụng nghiệp vụ ghi chỉ số cho phòng/hợp đồng có `electricityBillingMethod`/`waterBillingMethod = NOT_BILLED` (VD: căn hộ cho thuê nguyên căn, khách tự trả điện/nước trực tiếp cho toà nhà — không bắt buộc chỉ số nhận/trả phòng cho trường hợp này). |
