...
- **Thành phần chính:** **2 thứ tự chọn Nhà/phòng và Tenant (xem `BR-CTR-13`):** (a) **Nhà/phòng trước (mặc định):** chọn Nhà/Dãy trọ → chọn **nhiều phòng đang Empty** (multi-select, chỉ trong nhà đã chọn) → mục chọn Tenant chỉ hiện **Tenant Pool đã lọc theo đúng nhà này**; Tenant Pool rỗng → nút "Thêm nhanh" → T-04 (Nhà tự điền sẵn, ẩn field). (b) **Tenant trước:** chọn Tenant từ **toàn bộ Tenant Pool** đang có quyền truy cập → mục chọn Nhà/phòng tự **lọc lại chỉ còn đúng Nhà của Tenant đã chọn**. Sau khi đã chọn đủ Nhà/phòng + Tenant (theo chiều nào cũng vậy): **kiểm tra & bắt buộc ghi chỉ số nhận phòng (MOVE_IN) cho từng phòng chưa có** (mở inline hoặc điều hướng nhanh sang màn ghi chỉ số rồi quay lại), Ngày bắt đầu, Kỳ hạn, Tiền cọc (cho cả hợp đồng), Tiền thuê/tháng (cho cả hợp đồng, mặc định gợi ý = tổng giá tham khảo các phòng), phương thức + đơn giá điện/nước (theo chỉ số/khoán/không thu), Chu kỳ thu tiền nhà (số tháng + kỳ neo), Ngày cố định hạn thanh toán, Phí dịch vụ (phương thức Flat — nhập thẳng số tiền/tháng — hoặc By area — tự tính = tổng diện tích các phòng × đơn giá/m², cho sửa tay; xem DECISIONS.md Đợt 27), Phí định kỳ (từng dòng, "+"), Phạt trễ hạn (optional), Môi giới (optional), nút Lưu/Huỷ.
...
- **Hành động & điều hướng:** Xác nhận → ghi settlement vào `tb_contract` (kèm `settlementConfirmedAt`) → `status=Ended` → mọi phòng trong hợp đồng chuyển "Empty" → quay lại H-04 (phòng đầu tiên) hoặc T-02. **Không tự tạo hoá đơn cuối** (chốt 11/09/2026, xem [DECISIONS.md](DECISIONS.md)) — màn chỉ ghi lại số liệu đối soát (`unpaidInvoicesTotal`/`damageDeduction`/`refundAmount`), nếu còn khoản cần thu/hoàn thì chủ nhà tự tạo hoá đơn riêng ở B-0x.
