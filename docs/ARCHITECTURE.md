...
- **Auth**: Supabase Auth cho **2 loại tài khoản app**: Landlord (tự đăng ký, SĐT + OTP + mật khẩu — `signInWithOtp`/`verifyOtp`) và Manager (**do Landlord tạo trực tiếp**, đăng nhập bằng SĐT + mật khẩu do Landlord cấp, không tự đăng ký). OTP gửi qua **Send SMS Hook** tuỳ chỉnh, cắm nhà cung cấp SMS brandname Việt Nam (eSMS/Speedsms — xem [REQUIREMENTS.md](REQUIREMENTS.md) INT-02/INT-03). **Tenant không có `auth.users` row** trong Phase 1 — chỉ là bản ghi dữ liệu (`tenant` table), không đăng nhập được.
