...
- **manager_account** — tài khoản phụ do Landlord tạo (**không** gắn `auth.users` kiểu tự đăng ký — được Landlord khởi tạo trực tiếp): `phone` (PK, own login), `passwordHash`, `fullName`, `status` (active/disabled), `note` (optional). Tạo bởi 1 `landlord_account` (`created_by_landlord_id`).
