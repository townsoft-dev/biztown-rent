// invoice_image — phần thân dùng chung cho các hàm phục vụ ảnh hoá đơn.
//
// ẢNH hoá đơn chi tiết cho NGƯỜI THUÊ, mở bằng đúng một đường dẫn sạch:
//
//   GET /functions/v1/i/<ma_tra_cuu>        ← đường dẫn CHÍNH, gửi trong SMS
//   GET /functions/v1/invoice/<ma_tra_cuu>  ← giữ cho link đã gửi 18/09 không chết
//
// Tên hàm rút xuống một chữ `i` để tin nhắn xuống còn ĐÚNG 160 ký tự — tức 1
// đoạn SMS thay vì 2, giảm nửa tiền. Mỗi ký tự thừa ở đây là tiền thật: xem
// `src/lib/core/invoice_message.dart` để biết ngân sách ký tự được chia thế nào.
//
...
import { buildVietQrPayload } from "./vietqr.ts";
import { bankByBin } from "./invoice_message.ts";
import { buildInvoiceSvg, type Fee, type InvoiceImageData } from "./invoice_image_render.ts";
...
export const invoiceImageHandler = {
